...
const Details = () => {
  const [article, setArticle] = useState({ title: '', body: [], slug: '', author: '' });
  ...
  return (
    <div className="article">
      ...
      >{article.title}</motion.h2>
      <motion.article dangerouslySetInnerHTML={{ __html }}
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
        transition={{ ease: 'easeInOut', duration: 0.2, delay: 0.4 }}
      ></motion.article>
      <motion.p className="author"
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
      ...
      >
        <Link to='/articles' className="btn">Back</Link>
      </motion.span>
    </div>
  );
}
